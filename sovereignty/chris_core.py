...
class Chris:
    ...
    def __init__(self):
        ...
        print(f"[CHRIS] Sovereign core v3.0 online --- {self.boot_time}")
        print(f"[CHRIS] Brain: {'ACTIVE' if self.brain.alive else 'OFFLINE'}")
        print(f"[CHRIS] Inbox: {INBOX}")
        print(f"[CHRIS] ntfy: {NTFY_TOPIC}")
            ...
            for line in resp.iter_lines():
                if line:
                    ...
                    if m.get("event") == "message":
                        ...
                        print(f"[CHRIS] New message from ntfy: {m.get('message')}")
        except Exception as e:
            print(f"[CHRIS] ntfy poll error: {e}")
