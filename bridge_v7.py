...
Endpoints (all support GET with query params OR POST with JSON body):
  ...
  GET  /status          — Detailed status (auth required)
  POST /exec            — Execute shell command
  POST /toast           — Show toast notification
  POST /vibrate         — Vibrate device
  POST /speak           — Text-to-speech
  POST /listen          — Speech-to-text
  POST /notify          — Push ntfy notification
  POST /push_state      — Manually trigger Supabase state push
  GET  /exec            — Execute command (GET, query: cmd=)
  GET  /toast           — Toast (GET, query: msg=)
  GET  /vibrate         — Vibrate (GET, query: duration=)
  GET  /speak           — Speak (GET, query: msg=)
...
def supabase_upsert(table: str, payload: dict) -> dict:
    if not SUPABASE_KEY:
        log.debug("SUPABASE_KEY not set — skipping upsert to %s", table)
...
def supabase_insert(table: str, payload: dict) -> dict:
    if not SUPABASE_KEY:
        log.debug("SUPABASE_KEY not set — skipping insert to %s", table)
        return {"skipped": "no_key"}
    url = f"{SUPABASE_URL}/rest/v1/{table}"
    headers = _supabase_headers()
        ...
        while not _stop_event.is_set():
            try:
                ...
                if self._consecutive_failures >= 5:
                    log.critical("5 consecutive heartbeat failures — alerting.")
                    try:
                        ntfy_push(
                            f"CRITICAL: {DEVICE_ID} heartbeat failing — {self._consecutive_failures} consecutive errors!",
                ...
                if not tunnel_ok:
                    ...
                    if self._failures >= 3:
                        msg = (f"\u26A0\uFE0F {DEVICE_ID}: Tunnel health check failed "
                               f"{self._failures}x — check cloudflared.")
                        log.critical(msg)
...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not _check_auth(self):
            self._send_error_json(401, "Unauthorized — provide valid auth token.")
            return
    ...
    def do_POST(self):
        ...
        # ── Auth required for all POST routes ──
